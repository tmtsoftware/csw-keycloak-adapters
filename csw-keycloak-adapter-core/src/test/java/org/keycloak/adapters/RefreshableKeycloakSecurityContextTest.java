...
public class RefreshableKeycloakSecurityContextTest {
	...
	public void testSerialization() throws Exception {
		...
		assertEquals(encoded, ctx.getTokenString());
		assertEquals(encodedIdToken, ctx.getIdTokenString());
		assertEquals("111", token.getId());
		assertEquals("111", token.getId());
		assertTrue(token.getResourceAccess("foo").isUserInRole("admin"));
		assertTrue(token.getResourceAccess("bar").isUserInRole("user"));
		assertEquals("joe@email.cz", idToken.getEmail());
		assertEquals("acme", ctx.getRealm());
		ois.close();
	}
}
